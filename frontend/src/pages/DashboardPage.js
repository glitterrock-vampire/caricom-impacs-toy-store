  ...
  const fetchDashboardData = useCallback(debounce(async () => {
    ...
    try {
      ...
      const [statsRes, customersRes, ordersRes] = await Promise.all([
        retryWithBackoff(() => dashboardService.getDashboardStats()),
        retryWithBackoff(() => api.get('/api/customers?limit=100', config)),
        retryWithBackoff(() => dashboardService.getRecentOrders())
      ]);
